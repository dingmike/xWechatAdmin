            ...
            <div class="contentBox task" v-for="(task, tindex) in material.content" :class="{'content-active': showContentIndex==(tindex+3)}">
              ...
              <template v-if="task.type == 'task'">
                ...
                <div class="control is-horizontal">
                  ...
                  <div class="control">
                    <div class="control">
                      <label-select-tour :root="1" :el="task" :value="task.targetDefault" :selectCallback="selectCallback_tour" />
                    </div>
                  </div>
                </div>
                <div class="control is-horizontal">
...
//      return window.location.protocol + '//' + 'lanmatest.xinjijiaoyu.com' + '/api/user/upload'
  ...
  created () {
    ...
    this.id = id
    if (id) {
      api.material_tour.find(this.$route.params.id)
        .then((material) => {
          this.defaults = {
            age: material.age,
          ...
          material.content.map(item => {
            if (item.type === 'task' && item.target) {
              item.targetDefault = item.target
            }
            return item
          })
  ...
  methods: {
    ...
    submit () {
      ...
      this.countQuestionsTarget()
      if (id) {
        api.material_tour.edit(id, this.material)
          .then((res) => {
        ...
        api.material_tour.add(this.material)
          .then((res) => {
    ...
    countQuestionsTarget () {
      var theme = []
      var contents = this.material.content.filter(function (item) {
        return item.type === 'task'
      })
      for (let i = 0; i < contents.length; i++) {
        theme.push(contents[i].target)
      }
      this.material.theme = _.uniq(theme)
      console.log(_.uniq(theme))
    },
    validateInfo () {
      var error = {
        'title': '请输入课程名称',
    ...
    validateContent () {
      for (var i = 0; i < this.material.content.length; i++) {
        if (this.material.content[i].type === 'summary' && this.material.content[i].content === '') {
          ...
          return false
        } else if (this.material.content[i].type === 'task' && !this.material.content[i].target) {
          helper.showError('' + (i + 1) + '.任务卡目标不能为空')
          return false
