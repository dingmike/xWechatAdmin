              ...
              <tab-pane label="课程信息" :selected="tab === 'info'">
                <div class="block">
                  ...
                  <div class="control is-horizontal">
                    ...
                    <div class="control">
                      <div class="control">
                        <img v-if="course.cover" :src="course.cover" class="cover" @click="bigImgShow(course.cover)" style="width:160px;height:90px;">
...
<!--
                  <div class="control is-horizontal">
                    ...
                    <div class="control">
                      <div class="control">
                        <textarea class="textarea" v-model="course.content" :disabled="disabled" type="text" placeholder=""></textarea>
                      </div>
                    </div>
                  </div> -->
              		          		...
              		          		<div v-if="itemChild.type === 'photo'" class="answersImg">
              		          			<ul>
              		          				<li v-for="itemImg in itemAnswers.answers[index][indexChild]">
              		          					<img :src="countSmallUrl(itemImg, 'x1')" @click="bigImgShow(itemImg)" />
              		          				</li>
              		          			</ul>
              		          		</div>
              				...
              				<div class="itemChildList">
              				  <div class="itemChild">
              				    <div>
	              				    ...
	              				    <span class="imgBox" v-for="item,index in coursePics" v-if="coursePics && coursePics.length != 0 && JSON.stringify(coursePics) != '{}'">
	              				    	<img :src="countSmallUrl(item, 'x1')" @click="bigImgShow(item)" />
	              				    	<img :src="closeImg" class="imgClose" @click="delCoursePicsImg(index)" />
	              				    </span>
...
//      return window.location.protocol + '//' + 'lanmatest.xinjijiaoyu.com' + '/api/user/upload'
  ...
  methods: {
    ...
    getMaterialList() {
      ...
      api.material.getList(params).then((data) => {
    ...
    validate () {
      if (String(this.course.labelScene) === '62') {
        if (!this.course.lesson_site) {
          ...
          return false
        } else if (!this.course.cover) {
          helper.showError('请上传封面')
          return false
        } else if (!this.course.content || this.course.content === '<p><br></p>') {
          helper.showError('请填写课程卖点')
          return false
          ...
          helper.showError('请填写上课地点')
          return false
        } else if (!this.course.total || !String(this.course.total).match(/^[0-9]*[1-9][0-9]*$/)) {
          helper.showError('请设置课程人数上限（只支持输入数字）')
          return false
        }
      } else {
        if (!this.course.duration) {
          helper.showError('请正确设置上课时间')
          return false
        } else if (!this.course.title) {
          helper.showError('请填写标题')
          return false
...
//      console.log('index:' + index + 'indexchild:' + index, indexChild + 'length:' + taskListLi.length)
    ...
    imagechanged (files) {
      ...
      if (files.length > 1) {
        this.forUploadImg(files)
      } else {
        let data
        if (files.size/1024 > 7 * 1024) {
          ChangeImgSize.photoCompress(files, {quality: 0.65}, function(form) {
            data = form
            console.log(data)
            api.photo.upload(data)
              .then((data) => {
                _this.coursePics.push(data.url)
                _this.uploadModal.count += 1
                _this.isUploadOk()
              })
              .catch(e => {
                _this.uploadModal.count += 1
    ...
    forUploadImg (files) {
      ...
      let data
      var _this = this
      if (files[this.uploadModal.count - 1].size/1024 > 7 * 1024) {
        ChangeImgSize.photoCompress(files[this.uploadModal.count - 1], {quality: 0.65}, function(form) {
          data = form
          console.log(data)
          api.photo.upload(data)
            .then((data) => {
        ...
        data = new window.FormData()
        data.append('file', files[this.uploadModal.count - 1])
        api.photo.upload(data)
          .then((data) => {
            this.coursePics.push(data.url)
            this.uploadModal.count += 1
