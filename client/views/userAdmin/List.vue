          ...
          <div class="columns">
            <div class="column">
              <p class="control has-addons">
                ...
                <span class="channelSelect">
                  <span class="select-item">{{ channelSelected.user_id + " " + channelSelected.name }}</span>
                  <a class="select-item" @click="selectChannel">{{ channelSelected.user_id ? '重新选择' : '选择渠道' }}</a>
                </span>
          ...
          <table class="table" id="tab">
            ...
            <tbody>
              <tr v-for="item in list">
                ...
                <td>
                  <a href="#" @click.prevent="goto('/userAdmin/userItemRecord/' + item.id)">消费记录</a>&nbsp;&nbsp;
                </td>
              </tr>
            </tbody>
          </table>
...
//    this.getRecordList(this.page)
      ...
      var params = {
        ...
        'channelId': this.channelSelected.user_id,
        'keyword': this.infoKW
      }
    ...
    getUserList (params) {
      api.userAdmin.getUserList(params)
        .then((data) => {
          var _this = this
          data.list.map(item => {
